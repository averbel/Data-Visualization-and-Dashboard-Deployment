...
with col1:
    año = st.selectbox("Año", ["Todos"] + sorted(df['Year'].unique()))
with col2:
...
with kpi2:
    ...
    st.metric("Tasa Admisión", f"{tasa:.1f}%")

with kpi3:
    ret = df_filtrado['Retention Rate (%)'].mean()
    st.metric("Retención", f"{ret:.1f}%")

with kpi4:
    sat = df_filtrado['Student Satisfaction (%)'].mean()
    st.metric("Satisfacción", f"{sat:.1f}%")
...
fig.add_trace(go.Scatter(x=tendencias['Year'], y=tendencias['Retention Rate (%)'],
                         name="Retención", line=dict(color='blue')), secondary_y=False)
fig.add_trace(go.Scatter(x=tendencias['Year'], y=tendencias['Student Satisfaction (%)'],
                         name="Satisfacción", line=dict(color='green')), secondary_y=True)

fig.update_layout(title="Evolución Anual", height=400)
...
if departamento == "Todos":
    ...
    fig_line = px.line(dept_evol, x='Year', y=dept_col,
                       title=f"Evolución de Matrícula - {departamento}",
                       markers=True)
...
with col1:
    fig_ret = px.line(df, x='Year', y='Retention Rate (%)', color='Term',
                     title='Retención por Semestre')
    st.plotly_chart(fig_ret, use_container_width=True)

with col2:
    fig_sat = px.line(df, x='Year', y='Student Satisfaction (%)', color='Term',
                     title='Satisfacción por Semestre')
    st.plotly_chart(fig_sat, use_container_width=True)

# Insights
st.subheader(" Recomendación Estratégica")
st.info("""
**Implementar programa de mentoría para estudiantes de Ciencias y Artes**

- **Justificación**: Estos departamentos muestran menor crecimiento en matrícula
- **Objetivo**: Mejorar retención y satisfacción estudiantil
